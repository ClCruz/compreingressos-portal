...
public class TarefaController implements Serializable{
    ...
    public String redirect(Tarefa task){
        TaskService taskService = TaskBPM.getRuntimeEngine(JsfUtil.getLogin()).getTaskService();
        if(task.getStatus().equals("Reserved")){
            taskService.start(task.getId(), JsfUtil.getLogin().getDsNome());
        }        
    ...
    public void releaseTask(Tarefa task) {
        TaskService taskService = TaskBPM.getRuntimeEngine(JsfUtil.getLogin()).getTaskService();
        taskService.release(task.getId(), JsfUtil.getLogin().getDsNome());
    ...
    public void completeTask(Tarefa task) {
        TaskService taskService = TaskBPM.getRuntimeEngine(JsfUtil.getLogin()).getTaskService();
        taskService.complete(task.getId(), JsfUtil.getLogin().getDsNome(), new HashMap<String, Object>());
        JsfUtil.addSuccessMessage("Tarefa " + task.getName() + " concluída.");
        retrieveTasks();
    }
    
    public void deleteTask(){
        KieSession taskService = TaskBPM.getRuntimeEngine(JsfUtil.getLogin()).getKieSession();
